...
<section class="content-section withu-home-watch">
    ...
    <?php if (!empty($watchRoomSummary)): ?>
        <?php $roomTitle = $watchRoomSummary['series_name'] ?: $watchRoomSummary['file_name']; $roomEpisode = !empty($watchRoomSummary['episode_number']) ? '第 ' . (int)$watchRoomSummary['episode_number'] . ' 集' : '当前影片'; $roomState = $watchRoomSummary['playback_state'] === 'playing' ? '正在播放' : '已暂停'; $roomCover = (string)($watchRoomSummary['cover_url'] ?? '') ?: ('/api/strm.php?action=img&id=' . (int)$watchRoomSummary['media_id']); $roomPlayUrl = '/watch_play.php?source=strm&id=' . (int)$watchRoomSummary['media_id'] . ((int)($watchRoomSummary['source_episode'] ?? 0) > 0 ? '&episode=' . (int)$watchRoomSummary['source_episode'] : ''); ?>
        <div class="withu-together-card">
            <img class="withu-together-poster" src="<?php echo e($roomCover); ?>" alt="">
            ...
            <div class="withu-together-action"><a href="<?php echo e($roomPlayUrl); ?>"><i class="fas fa-play"></i> <?php echo $watchRoomSummary['joined'] ? '继续一起看' : '加入一起看'; ?></a></div>
        </div>
    <?php else: ?>
