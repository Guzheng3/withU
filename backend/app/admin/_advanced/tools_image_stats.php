<?php
// 内联片段：图片体积 / 压缩 / 补齐工具（「图片统计」与「图片补齐」两个 tab 共用）。
// 依赖外部已注入的 $db（独立页与 settings.php 均会先完成鉴权）。
// 注意：AJAX 端点（action=stats / video_stats / optimize_batch 等）保留在 tools_image_stats.php 独立页，
// 本片段只负责 mode 表单提交逻辑、体积估算与页面渲染，返回 HTML 字符串。
