...
$moderation = withu_moderate_text($db, 'comment', 0, $content);
if ($moderation['blocked']) {
    jsonResponse(['success' => false, 'message' => '内容触发安全规则，已拦截并提交后台复核'], 403);
}
...
if (!empty($moderation['log_id'])) {
    withu_finish_moderation($db, (int)$moderation['log_id'], (int)$commentId);
}
