...
$albumLayout = in_array($album['layout_mode'] ?? 'grid', ['grid', 'waterfall', 'heart'], true) ? ($album['layout_mode'] ?? 'grid') : 'grid';
$albumMask = in_array($album['mask_type'] ?? 'none', ['none', 'circle', 'heart'], true) ? ($album['mask_type'] ?? 'none') : 'none';
...
<section class="album-page-section">
    <div class="album-page-main">
        <?php if (!empty($album['location_name'])): ?>
            <div class="album-location glass-card"><i class="fas fa-location-dot"></i> <?php echo e($album['location_name']); ?><?php if (empty($currentUser)): ?><span>（大概位置）</span><?php endif; ?></div>
        <?php endif; ?>
        ...
        <?php else: ?>
            ...
            <?php else: ?>
                ...
                ?>
                <div class="album-photo-wall album-layout-<?php echo e($albumLayout); ?> album-mask-<?php echo e($albumMask); ?>">
...
.album-layout-heart .album-photo-item {
...
.album-mask-circle .album-photo-media,
.album-mask-circle .album-photo-media img {
    border-radius: 50%;
}

.album-mask-heart .album-photo-media,
.album-mask-heart .album-photo-media img {
    clip-path: polygon(50% 96%, 12% 58%, 4% 38%, 8% 18%, 24% 8%, 38% 12%, 50% 26%, 62% 12%, 76% 8%, 92% 18%, 96% 38%, 88% 58%);
}
