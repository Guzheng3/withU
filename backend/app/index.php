...
if ($currentUser) {
    try {
        // STRM 媒体库（withUstrm）作为唯一影视数据源：首页只做预览，
        // 拉取第一页即可，避免把整库加载进 PHP 和浏览器。
        $strmJwtPath = dirname(__DIR__) . '/runtime/strm/jwt.txt';
        $strmSecret = is_file($strmJwtPath) ? trim((string)file_get_contents($strmJwtPath)) : '';
        $strmCache = [];
        $strmInternal = static function (string $path) use (&$strmCache, $strmSecret): array {
            if (isset($strmCache[$path])) return $strmCache[$path];
            if ($strmSecret === '') return $strmCache[$path] = [];
            ...
            $signature = $b64u(hash_hmac('sha256', $header . '.' . $payload, $strmSecret, true));
            $ch = curl_init('http://127.0.0.1:8081/api/media-library' . $path);
            ...
            return $strmCache[$path] = (($response['code'] ?? 0) === 200 && is_array($response['data'] ?? null)) ? $response['data'] : [];
        };
        // "最新添加"：withUstrm 媒体列表第一页
        $strmList = $strmInternal('?page=1&pageSize=18');
        $listItems = (array)($strmList['items'] ?? []);
        $listRows = [];
        foreach ($listItems as $item) {
            ...
            $listRows[] = [
                'id' => $mid,
                'series_name' => (string)($item['title'] ?? 'strm 媒体'),
                'series_key' => 'strm-title-' . preg_replace('/\s+/u', '', mb_strtolower((string)($item['title'] ?? 'strm 媒体'), 'UTF-8')),
                'file_name' => (string)($item['title'] ?? ''),
                'episode_number' => 0,
                'resolution' => $type === 'tv' ? '剧集' : '电影',
                'cover_url' => $poster,
                'rating' => isset($item['voteAverage']) && (float)$item['voteAverage'] > 0 ? (string)round((float)$item['voteAverage'], 1) : '',
            ];
        }
        ...
        foreach ($historyRows as $historyRow) {
            ...
            $item = array_merge($mediaRow, $historyRow);
        ...
        $activeRoom = $db->fetch("SELECT room_code, media_id, source, source_episode, playback_state, current_position_ms, speed, last_sync_at FROM watch_rooms WHERE room_code = :code AND source = 'strm' LIMIT 1", ['code' => 'WithU Watch']);
        if ($activeRoom) {
            $meta = $strmInternal('/' . (int)$activeRoom['media_id']);
            if ($meta) {
                $episodeId = (int)($activeRoom['source_episode'] ?? 0);
                $episode = null;
                foreach ((array)($meta['episodes'] ?? []) as $candidate) {
                    if ((int)($candidate['id'] ?? 0) === $episodeId) { $episode = $candidate; break; }
                }
                $title = (string)($meta['title'] ?? 'strm 媒体');
                $activeRoom['file_name'] = $episode ? (string)($episode['sourceFileName'] ?? ('第 ' . (int)($episode['episodeNo'] ?? 0) . ' 集')) : $title;
                $activeRoom['series_name'] = $title;
                $activeRoom['episode_number'] = $episode ? (int)($episode['episodeNo'] ?? 0) : 0;
                $activeRoom['cover_url'] = (string)($meta['posterUrl'] ?? '');
