<template>
  <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <button type="button" class="mb-3 text-sm text-blue-600 hover:text-blue-700" @click="goBack">
          ← 返回任务管理
        </button>
        <h1 class="text-2xl font-semibold text-gray-800">手动刮削</h1>
    ...
    <div v-if="loading" class="card py-20 text-center text-gray-500">正在读取任务目录...</div>
    <div v-else-if="pageError" class="rounded-xl border border-red-200 bg-red-50 p-5 text-red-600">
      {{ pageError }}
    ...
    <div v-else>
      ...
      >
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div class="min-w-0">
            <div class="flex items-center gap-2">
              ...
              <span class="text-xs text-gray-400">#{{ scrapingJob.id }} · {{ jobStageLabel(scrapingJob.stage) }}</span>
            </div>
            <p class="mt-2 text-sm text-gray-600">{{ scrapingJob.message }}</p>
            <p
            ...
            <p v-if="scrapingJob.errorMessage" class="mt-2 break-all text-sm text-red-600">
              {{ scrapingJob.errorMessage }}
            </p>
            <p v-if="isJobActive" class="mt-2 text-xs text-amber-600/65">
        ...
        <div v-if="isJobActive" class="mt-4 h-2 overflow-hidden rounded-full bg-gray-200">
          <div
            class="h-full rounded-full bg-blue-400 transition-all duration-500"
            :style="{ width: `${scrapingJob.progress || 0}%` }"
          />
        </div>
        <div v-if="isJobActive" class="mt-1 text-right text-xs text-gray-400">
      ...
      <div class="grid gap-6 lg:grid-cols-[minmax(320px,0.85fr)_minmax(0,1.4fr)]">
        <section class="card min-h-[560px]" :class="{ 'pointer-events-none opacity-60': isJobActive }">
          <div class="mb-4">
            <h2 class="font-medium text-gray-800">任务目录</h2>
            <p class="mt-1 text-xs text-gray-500">选择包含一个电影或一部剧集的目录</p>
          </div>
        ...
        <div v-if="!selectedDirectory" class="flex h-full min-h-[480px] items-center justify-center text-center">
          <div>
            ...
            <p class="mt-4 text-gray-500">请从左侧选择一个媒体目录</p>
          </div>
        </div>

        <div v-else-if="previewError" class="rounded-xl border border-red-200 bg-red-50 p-5 text-red-600">
          {{ previewError }}
        </div>

        <div v-else-if="!preview" class="flex h-full min-h-[480px] items-center justify-center text-center">
          <div>
            <p class="font-medium text-gray-700">{{ selectedDirectory.name }}</p>
            <p class="mt-2 break-all font-mono text-xs text-gray-400">{{ selectedDirectory.path }}</p>
        ...
        <div v-else-if="!preview.matched" class="mx-auto max-w-xl space-y-5 py-8">
          <div class="rounded-xl border border-amber-200 bg-amber-50 p-5">
            <h2 class="font-medium text-amber-600">没有找到匹配的 TMDB 条目</h2>
            <p class="mt-2 text-sm leading-6 text-amber-600/65">
              {{ preview.matchMessage }}
            </p>
          </div>

          <div class="rounded-xl border border-gray-200 bg-gray-50 p-5">
            <h3 class="font-medium text-gray-800">手动修正搜索条件</h3>
            <div class="mt-4 grid gap-4 sm:grid-cols-[minmax(0,1fr)_120px]">
              <div>
                <label for="manualTitle" class="mb-1.5 block text-xs text-gray-500">标题</label>
                <input
                  id="manualTitle"
                  v-model="manualTitle"
                  type="text"
              ...
              <div>
                <label for="manualYear" class="mb-1.5 block text-xs text-gray-500">年份</label>
            ...
            <div>
              <label for="manualTmdbId" class="mb-1.5 block text-xs text-gray-500">TMDB ID</label>
              <input
                id="manualTmdbId"
            ...
            <p v-if="manualSearchError" class="mt-3 text-sm text-red-600">{{ manualSearchError }}</p>
            <button
              type="button"
        ...
        <div v-else class="space-y-6">
          ...
          <div v-if="showManualSearch" class="rounded-xl border border-blue-200 bg-blue-500/[0.06] p-4">
            <div class="grid gap-3 sm:grid-cols-[minmax(0,1fr)_110px_130px_auto] sm:items-end">
              <div>
                <label for="matchedManualTitle" class="mb-1.5 block text-xs text-gray-500">标题</label>
                <input id="matchedManualTitle" v-model="manualTitle" type="text" class="input-field">
              </div>
              <div>
                <label for="matchedManualYear" class="mb-1.5 block text-xs text-gray-500">年份</label>
                <input id="matchedManualYear" v-model="manualYear" type="text" class="input-field">
              </div>
              <div>
                <label for="matchedManualTmdbId" class="mb-1.5 block text-xs text-gray-500">TMDB ID</label>
          ...
          <div class="flex flex-col gap-5 sm:flex-row">
            <img
              ...
              :alt="preview.title"
              class="h-64 w-44 shrink-0 rounded-xl bg-gray-50 object-cover shadow-xl"
            >
            <div class="min-w-0">
              <div class="flex flex-wrap items-center gap-2">
                <span class="badge-success">{{ preview.mediaType === 'movie' ? '电影' : '电视剧' }}</span>
                <span class="text-xs text-gray-400">TMDB #{{ preview.tmdbId }}</span>
              </div>
              <h2 class="mt-3 text-2xl font-semibold text-gray-800">{{ preview.title }}</h2>
              <p v-if="preview.originalTitle && preview.originalTitle !== preview.title" class="mt-1 text-sm text-gray-500">
              ...
              <p class="mt-4 line-clamp-6 text-sm leading-6 text-gray-600">
          ...
          <div class="rounded-xl border border-gray-200 bg-gray-50 p-4">
            <label class="flex cursor-pointer items-start gap-3">
              ...
              <span>
                <span class="block text-sm font-medium text-gray-800">重命名媒体目录和文件</span>
                <span class="mt-1 block text-xs leading-5 text-amber-600/65">
            ...
            <p v-if="isTaskRoot && renameMedia" class="mt-3 text-xs text-red-600">
              当前选择的是任务根目录，不能重命名。请选择其下的媒体目录。
            </p>
          </div>

          <div v-if="renameMedia" class="space-y-3">
            <h3 class="text-sm font-medium text-gray-700">重命名预览</h3>
            <div class="rounded-lg bg-gray-50 p-3 text-sm">
              <span class="text-gray-500">媒体目录：</span>
              <span class="break-all font-mono text-gray-700">{{ selectedDirectory.name }}</span>
              <span class="mx-2 text-gray-400">→</span>
              <span class="break-all font-mono text-emerald-600">{{ preview.proposedDirectoryName }}</span>
            </div>
            <div v-if="preview.proposedDirectoryRenames?.length" class="space-y-1">
              <div class="px-3 text-xs text-gray-500">季目录</div>
              <div
                v-for="item in preview.proposedDirectoryRenames"
                :key="item.sourcePath"
                class="grid gap-1 rounded-lg px-3 py-2 text-xs sm:grid-cols-[1fr_auto_1fr]"
              >
                <span class="break-all font-mono text-gray-500">{{ item.sourceName }}</span>
                <span class="hidden text-gray-400 sm:block">→</span>
                <span
                  class="break-all font-mono"
                  :class="item.sourceName === item.targetName ? 'text-gray-400' : 'text-emerald-600'"
                >
            ...
            <div class="max-h-48 space-y-1 overflow-y-auto">
              <div class="px-3 text-xs text-gray-500">媒体文件</div>
              <div
              ...
              >
                <span class="break-all font-mono text-gray-500">{{ item.sourceName }}</span>
                <span class="hidden text-gray-400 sm:block">→</span>
                <span class="break-all font-mono text-blue-600">{{ item.targetName }}</span>
              </div>
            </div>
          </div>

          <div>
            <h3 class="text-sm font-medium text-gray-700">将上传到所选目录</h3>
            <div class="mt-2 flex flex-wrap gap-2">
              <span
                v-for="file in displayedGeneratedFiles"
                :key="file"
                class="rounded-lg border border-gray-200 bg-gray-50 px-3 py-1.5 font-mono text-xs text-gray-600"
              >
                {{ file }}
              </span>
            </div>
          ...
          <div v-if="executeResult" class="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-600">
