    ...
    <template v-else>
      ...
      <div class="card mb-6 animate-fade-in">
        <div class="flex items-center justify-between mb-4">
          <div>
            <h3 class="text-lg leading-6 font-medium text-gray-800">配置信息</h3>
        ...
        <div class="mt-5 border-t border-gray-100 pt-5" v-if="configInfo">
          <dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
            <div>
              <dt class="text-sm text-gray-500">用户名</dt>
              <dd class="mt-1 text-sm text-gray-800">{{ configInfo.username }}</dd>
            </div>
            <div>
              <dt class="text-sm text-gray-500">Base URL</dt>
              <dd class="mt-1 text-sm text-gray-800 break-all font-mono">{{ configInfo.baseUrl }}</dd>
            </div>
            <div>
              <dt class="text-sm text-gray-500">Base Path</dt>
              <dd class="mt-1 text-sm text-gray-800">{{ configInfo.basePath || '/' }}</dd>
            </div>
            <div>
              <dt class="text-sm text-gray-500">创建时间</dt>
              <dd class="mt-1 text-sm text-gray-800">{{ formatDate(configInfo.createdAt) }}</dd>
            </div>
          </dl>
        </div>
      </div>
      ...
      <!-- 目录浏览区域 -->
      <div class="card mb-6 animate-fade-in" style="animation-delay: 0.05s">
        <div class="flex items-center justify-between mb-4">
          <div>
            <h3 class="text-lg leading-6 font-medium text-gray-800">目录浏览</h3>
            <p class="mt-1 text-sm text-gray-500">浏览 OpenList 目录，创建任务时可直接选择路径</p>
          </div>
        </div>
        <ConfigDirectoryBrowser
          v-if="configId"
          :config-id="configId"
          @add-task="onBrowseAddTask"
        />
      </div>
      ...
      <div class="card animate-fade-in" style="animation-delay: 0.1s">
        <div class="flex items-center justify-between mb-6">
          <div>
            <h3 class="text-lg leading-6 font-medium text-gray-800">任务管理</h3>
        ...
        <div class="space-y-4" v-if="tasks.length > 0">
          <div class="card" v-for="task in tasks" :key="task.id">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-3">
              <h4 class="text-lg font-medium text-gray-800">{{ task.taskName }}</h4>
              <div class="flex items-center space-x-2 flex-wrap gap-y-2">
                <span :class="task.isActive ? 'badge-success' : 'badge-neutral'" class="text-xs">
                  {{ task.isActive ? '启用' : '禁用' }}
                </span>
                <button
                  class="btn-icon text-amber-600 hover:text-amber-600"
                ...
                <button class="btn-icon text-emerald-600 hover:text-emerald-700" @click="showExecuteModal(task.id)" :disabled="generatingStrm[task.id]" title="立即执行">
                ...
                <button class="btn-icon text-red-600 hover:text-red-700" @click="deleteTask(task.id)" title="删除">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="grid grid-cols-1 gap-x-4 gap-y-3 sm:grid-cols-2">
              <div>
                <dt class="text-sm text-gray-500">路径</dt>
                <dd class="mt-1 text-sm text-gray-700 break-all font-mono">{{ task.path }}</dd>
              </div>
              <div>
                <dt class="text-sm text-gray-500">媒体库类型</dt>
                <dd class="mt-1 text-sm text-gray-700">{{ libraryTypeLabel(task.libraryType) }}</dd>
              </div>
              <div>
                <dt class="text-sm text-gray-500 flex items-center gap-1">
                  STRM路径
                  <span class="text-xs text-gray-400 font-normal">（点击查看媒体）</span>
                </dt>
                <dd class="mt-1 text-sm text-gray-700 break-all font-mono">
                  <a
                    :href="`/media-library?taskId=${task.id}`"
                    target="_blank"
                    rel="noopener"
                    class="text-pink-600 hover:text-pink-700 hover:underline inline-flex items-center gap-1 break-all"
                    title="在新窗口查看该任务生成的 STRM 媒体（支持海报预览、播放与详情）"
                  >
                    {{ task.strmPath }}
              ...
              <div>
                <dt class="text-sm text-gray-500">定时任务</dt>
                <dd class="mt-1 text-sm text-gray-700">{{ task.cron || '未设置' }}</dd>
              </div>
              <div>
                <dt class="text-sm text-gray-500">上次执行</dt>
                <dd class="mt-1 text-sm text-gray-700">{{ formatDate(task.lastExecTime) }}</dd>
              </div>
              <div>
                <dt class="text-sm text-gray-500">媒体库刷新</dt>
                <dd class="mt-1 text-sm text-gray-700">{{ taskRefreshLabel(task) }}</dd>
              </div>
            </div>

            <div class="mt-3 flex items-center space-x-4 flex-wrap gap-y-2">
              <label class="flex items-center text-sm text-gray-600">
                <input type="checkbox" :checked="task.needScrap" disabled class="mr-2 h-4 w-4 rounded border-gray-300 bg-gray-50 text-blue-600">
                需要刮削
              </label>
              <label class="flex items-center text-sm text-gray-600">
                <input type="checkbox" :checked="task.autoRenameMedia" disabled class="mr-2 h-4 w-4 rounded border-gray-300 bg-gray-50 text-blue-600">
                自动重命名媒体
              </label>
              <label class="flex items-center text-sm text-gray-600">
                <input type="checkbox" :checked="task.isIncrement" disabled class="mr-2 h-4 w-4 rounded border-gray-300 bg-gray-50 text-blue-600">
                增量更新
              </label>
              <label class="flex items-center text-sm text-gray-600">
                <input type="checkbox" :checked="task.skipInvalidStructure" disabled class="mr-2 h-4 w-4 rounded border-gray-300 bg-gray-50 text-blue-600">
                跳过异常目录
              </label>
            </div>

            <div class="mt-3" v-if="task.renameRegex">
              <dt class="text-sm text-gray-500">STRM 文件名正则表达式</dt>
              <dd class="mt-1 text-sm text-gray-700 font-mono bg-gray-50 px-3 py-2 rounded break-all">{{ task.renameRegex }}</dd>
            </div>
        ...
        <div class="text-center py-12" v-else>
          ...
          <h3 class="text-lg font-medium text-gray-800 mb-2">暂无任务配置</h3>
        ...
        <div class="flex items-center justify-center min-h-screen p-4">
          ...
          >
            ...
            <form @submit.prevent="submitTask" class="flex min-h-0 flex-1 flex-col">
              ...
              <div>
                <label class="block text-sm text-gray-600 mb-2">任务路径 *</label>
                <div class="flex gap-2">
                  <input v-model="taskForm.path" type="text" required class="input-field flex-1" placeholder="请输入OpenList中的媒体路径">
                  <button type="button" class="btn-secondary shrink-0" @click="showPathPicker = !showPathPicker">
                    <svg class="mr-1 inline-block h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7a2 2 0 012-2h4l2 2h8a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V7z" />
                    </svg>
                    {{ showPathPicker ? '收起' : '选择目录' }}
                  </button>
                </div>
                <p class="mt-1 text-xs text-gray-400">可点击「选择目录」浏览并选中 OpenList 文件夹，自动填入路径</p>
                <div v-if="showPathPicker" class="mt-2">
                  <ConfigDirectoryBrowser
                    :config-id="configId"
                    select-mode
                    @select="onPathPickerSelect"
                  />
                </div>
              </div>

              <div>
                <label class="block text-sm text-gray-600 mb-2">媒体库类型 *</label>
                ...
                <p v-if="taskForm.libraryType === 'auto'" class="mt-2 text-xs text-amber-600/80">
                  不建议使用自动识别；该类型仅用于兼容旧任务，新任务请选择电影、电视剧或动画。
                </p>
              </div>

              <div>
                <label class="block text-sm text-gray-600 mb-2">STRM路径</label>
                <div class="flex">
                ...
                <p class="mt-1 text-xs text-gray-400">前缀 /app/backend/strm/ 固定不可修改；新建任务时默认与任务名称一致</p>
              </div>

              <div>
                <label class="block text-sm text-gray-600 mb-2">定时任务表达式</label>
                <input v-model="taskForm.cron" type="text" placeholder="例如: 0 15 10 ? * *" class="input-field">
                <p class="mt-1 text-xs text-gray-400">Cron表达式格式，留空表示不启用定时任务；新建任务默认每 6 小时执行一次</p>
              </div>

              <div>
                <div class="flex items-center justify-between mb-2">
                  <label class="block text-sm text-gray-600">STRM 文件名正则表达式</label>
                  <button type="button" @click="showRenameRegexHelp = !showRenameRegexHelp" class="text-gray-500 hover:text-blue-400 transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <p class="mt-1 text-xs text-gray-400">仅修改本地生成的 STRM 文件名，不修改 OpenList 源文件</p>

                <div v-if="showRenameRegexHelp" class="mt-3 p-4 bg-blue-50 border border-blue-200 rounded-xl">
                  <h4 class="text-sm font-medium text-blue-400 mb-2">使用说明</h4>
                  <div class="text-xs text-gray-600 space-y-2">
                    <p><strong>格式：</strong>原始模式|替换内容</p>
                    <p><strong>示例：</strong></p>
                    <ul class="list-disc list-inside ml-2 space-y-1">
                      <li>移除方括号：<code class="bg-gray-100 px-1 rounded">[\[\]()]|</code></li>
                      <li>空格转下划线：<code class="bg-gray-100 px-1 rounded">\s+|_</code></li>
                      <li>添加前缀：<code class="bg-gray-100 px-1 rounded">^|Movie_</code></li>
                    </ul>
                  </div>
                </div>
              </div>
              ...
              <div class="rounded-xl border border-gray-200 bg-gray-50 p-4 space-y-4">
                <div>
                  <label class="block text-sm text-gray-600 mb-2">任务完成后刷新媒体库</label>
                  ...
                  <p class="mt-1 text-xs text-gray-400">全量任务始终提交刷新；增量任务仅在媒体有变化时提交</p>
                </div>

                <div v-if="taskForm.mediaRefreshScope !== 'NONE'">
                  <label class="block text-sm text-gray-600 mb-2">媒体服务器 *</label>
                  ...
                  <p v-if="selectedMediaServer && !selectedMediaServer.active" class="mt-2 text-xs text-red-600">
                    ⚠️ 当前服务器已停用，请重新选择、启用配置或关闭刷新后再保存
                  </p>
                </div>

                <div v-if="taskForm.mediaRefreshScope === 'LIBRARY'">
                  <label class="block text-sm text-gray-600 mb-2">目标媒体库 *</label>
                  <select
                  ...
                  <p v-if="libraryConfirmedStale" class="mt-2 text-xs text-red-600">
                    ⚠️ 已保存的媒体库已不存在。系统不会按名称匹配，也不会回退为全部刷新，请重新选择后再保存。
                  </p>
                  <p v-else-if="libraryLoadState === 'error'" class="mt-2 text-xs text-amber-600">
              ...
              <div class="space-y-3">
                <label class="flex items-start cursor-pointer">
                  <input v-model="taskForm.needScrap" type="checkbox" class="mt-1 h-4 w-4 rounded border-gray-300 bg-gray-50 text-blue-600">
                ...
                >
                  <input
                    ...
                    class="mt-1 h-4 w-4 rounded border-gray-300 bg-gray-50 text-blue-600"
                  >
                  <span class="ml-2 text-sm text-gray-600">
                    普通任务自动重命名媒体
                ...
                <label class="flex items-center cursor-pointer">
                  <input v-model="taskForm.isIncrement" type="checkbox" class="h-4 w-4 rounded border-gray-300 bg-gray-50 text-blue-600">
                  <span class="ml-2 text-sm text-gray-600">增量更新</span>
                </label>

                <label class="flex items-center cursor-pointer">
                  <input v-model="taskForm.isActive" type="checkbox" class="h-4 w-4 rounded border-gray-300 bg-gray-50 text-blue-600">
                  <span class="ml-2 text-sm text-gray-600">启用任务</span>
                </label>
              </div>
              </div>

              <div class="flex shrink-0 justify-end gap-3 border-t border-gray-200 bg-gray-50 px-4 py-4 sm:px-6">
      ...
      <div v-if="showExecuteTaskModal" class="modal-overlay animate-fade-in">
        <div class="flex items-center justify-center min-h-screen p-4">
          <div class="modal-content animate-scale-in w-full max-w-md" @click.stop>
            <div class="flex items-center justify-between mb-6">
              <h3 class="text-xl font-semibold text-gray-800">选择执行模式</h3>
            ...
            <div class="space-y-4">
              <p class="text-sm text-gray-500">请选择任务执行模式：</p>

              <button @click="executeTask(currentTaskId, false)" class="w-full flex items-center justify-between p-4 bg-gray-50 hover:bg-gray-100 border border-gray-200 rounded-xl transition-all cursor-pointer">
                <div class="text-left">
                  <div class="font-medium text-gray-800">全量执行</div>
              ...
              <button @click="executeTask(currentTaskId, true)" class="w-full flex items-center justify-between p-4 bg-gray-50 hover:bg-gray-100 border border-gray-200 rounded-xl transition-all cursor-pointer">
                <div class="text-left">
                  <div class="font-medium text-gray-800">增量执行</div>
        ...
        <div class="flex min-h-screen items-center justify-center p-4">
          <div class="modal-content animate-scale-in flex max-h-[85vh] w-full max-w-3xl flex-col" @click.stop>
            <div class="mb-5 flex items-start justify-between gap-4">
              <div>
                <h3 class="text-xl font-semibold text-gray-800">目录结构检查</h3>
            ...
            <div v-if="checkingStructureTaskId" class="flex flex-1 flex-col items-center justify-center py-16">
              ...
              <p class="mt-4 text-sm text-gray-500">正在读取任务根目录...</p>
            </div>

            <div v-else-if="structureCheckError" class="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-600">
              {{ structureCheckError }}
            </div>
            ...
            <div v-else-if="structureCheckOverview" class="min-h-0 flex-1 overflow-y-auto pr-1">
              <div class="mb-4 rounded-xl border border-gray-200 bg-gray-50 p-4">
                <div class="text-xs text-gray-500">期望目录结构</div>
                <div class="mt-1 font-mono text-sm text-amber-600">{{ structureCheckOverview.expectedStructure }}</div>
                <div class="mt-2 break-all text-xs text-gray-400">任务根目录：{{ structureCheckOverview.rootPath }}</div>
              </div>

              <div v-if="!structureCheckOverview.supported" class="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-200">
              ...
              <template v-else>
                ...
                >
                  <div class="flex items-center justify-between gap-3">
                    <div>
                      <div class="font-medium text-red-600">任务根目录存在异常媒体文件</div>
                      <div class="mt-1 text-xs text-red-600/60">
                        根目录下的视频文件不属于任何第一层媒体目录
                      </div>
                    </div>
                    <span class="rounded-full bg-red-100 px-2.5 py-1 text-xs text-red-600">
                      {{ structureCheckOverview.rootFilesResult.invalidFileCount }} 个异常
                    </span>
                  </div>
                  <div class="mt-3 rounded-lg border border-gray-200 bg-gray-200 p-2">
                ...
                <div class="mb-3 flex items-center justify-between">
                  <div>
                    <h4 class="font-medium text-gray-800">第一层媒体目录</h4>
                    <p class="mt-1 text-xs text-gray-400">点击检查后，只递归扫描对应目录</p>
                  </div>
                  <span class="text-xs text-gray-400">{{ structureCheckOverview.directories.length }} 个目录</span>
                </div>
                ...
                <div v-if="structureCheckOverview.directories.length" class="space-y-3">
                  ...
                  >
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                      <div class="min-w-0">
                        <div class="truncate font-medium text-gray-800">{{ directory.name }}</div>
                        <div class="mt-1 truncate font-mono text-xs text-gray-400">{{ directory.path }}</div>
                      </div>
                      <button
                        type="button"
                    ...
                    <div v-if="structureDirectoryStates[directory.path]?.result" class="mt-3">
                      <div class="flex flex-wrap gap-2 text-xs">
                        <span class="rounded-md bg-gray-50 px-2 py-1 text-gray-500">
                      ...
                      <div
                        ...
                        class="mt-3 rounded-lg border border-gray-200 bg-gray-200 p-2"
                      >
                ...
                <div v-else class="rounded-xl border border-gray-200 bg-gray-50 p-5 text-center text-sm text-gray-500">
                  {{ structureCheckOverview.message }}
                </div>
              </template>
            </div>
...
import ConfigDirectoryBrowser from '~/components/ConfigDirectoryBrowser.vue'
...
const resetTaskForm = () => {
  taskForm.value = {
    taskName: '', path: '', strmPath: '/app/backend/strm', cron: '0 0 0/6 * * ?',
    libraryType: '', needScrap: true, autoRenameMedia: false, skipInvalidStructure: false,
...
const onPathPickerSelect = (item) => {
  if (item.type !== 'folder') return
  taskForm.value.path = item.path
  if (!taskForm.value.taskName) {
    taskForm.value.taskName = item.name
  }
  if (strmSubPath.value === '' || !strmSubPath.value) {
    strmSubPath.value = item.name
  }
}

const onBrowseAddTask = (item) => {
  if (item.type !== 'folder') return
  resetTaskForm()
