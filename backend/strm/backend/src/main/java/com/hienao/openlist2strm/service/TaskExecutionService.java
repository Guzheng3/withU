...
public class TaskExecutionService {
  ...
  private NotificationEvent executeTaskWithHandlerChain(
    ...
    for (OpenlistApiService.OpenlistFile videoFile : videoFiles) {
      ...
      if (strmFileExists(taskConfig, videoFile)) {
        strmSucceededCount++;
        indexMediaItem(taskConfig, openlistConfig, videoFile, fileContext);
      }
  ...
  private void indexMediaItem(
    ...
    try {
      String relativePath =
          strmFileService.calculateRelativePath(taskConfig.getPath(), videoFile.getPath());
      Path strmPath =
          strmFileService.resolveStrmFilePath(
              taskConfig.getStrmPath(), relativePath, videoFile.getName(), taskConfig.getRenameRegex());

      ScrapeOutcome scrapeOutcome = fileContext.getAttribute("scrapeOutcome");
      String mediaType = taskConfig.getLibraryType();
      String title = null;
      if (scrapeOutcome != null && scrapeOutcome.mediaTitle() != null) {
        title = scrapeOutcome.mediaTitle();
      }
      if (!StringUtils.hasText(mediaType) || "auto".equals(mediaType)) {
        com.hienao.openlist2strm.title.LocalParseResult parsed =
            com.hienao.openlist2strm.title.MediaPathParser.parse(
                videoFile.getName(), relativePath, taskConfig.getLibraryType());
        if (parsed != null) {
          if (!StringUtils.hasText(title)
              && parsed.getTitleCandidates() != null
              && !parsed.getTitleCandidates().isEmpty()) {
            title = parsed.getTitleCandidates().get(0);
          }
          if ("tv".equals(parsed.getMediaType())) {
            mediaType = "tv";
          } else if ("movie".equals(parsed.getMediaType())) {
            mediaType = "movie";
          }
        }
      }
      mediaLibraryService.recordGeneratedFile(
          taskConfig,
          openlistConfig,
          videoFile.getPath(),
          strmPath.toString(),
      ...
      log.warn("媒体库索引写入失败: {}, 错误: {}", videoFile.getPath(), e.getMessage());
    }
  }
