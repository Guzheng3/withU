...
public class EnhancedRegexPatterns {
  ...
  public static List<String> getEnhancedMovieRegexps() {
    return Arrays.asList(
        ...
        // 新增正则表达式 - 按优先级排序
        "^(?<title>[\\u4e00-\\u9fff]+.*?)[. _](?<year>19\\d{2}|20\\d{2})", // 中文电影名 + 年份
        "^(?<title>.+?)[. _](?<year>19\\d{2}|20\\d{2})[. _](?<title2>.*)$", // 年份在中间
        "^(?<title>[^.]+)[. ]+(?<year>19\\d{2}|20\\d{2})", // 多点分隔 + 年份
        "^(?<title>.+?)[. _](?<year>19\\d{2}|20\\d{2})$", // 年份在最后
        "^(?<title>.+?)[."
            + " _](?:1080p|720p|480p|2160p|4K|BluRay|DVD|HDTV|BDRip|x264|x265)", // 无年份，质量标记
        "^(?<title>[\\u4e00-\\u9fff]+.+?)(?:\\.(?:mp4|mkv|avi|rmvb))?$", // 中文无年份
        "^\\[(?<title>.+?)\\][. _](?<year>19\\d{2}|20\\d{2})", // 带方括号 + 年份
        "^(?<title>.+?)[. _]\\[(.+?)\\](?:[. _](?<year>19\\d{2}|20\\d{2}))?$", // 带内容，可选年份
        // [中文名].英文名.年份：[新僵尸先生].Mr.Vampire.1992...
        "^\\[(?<title>[^\\[\\]]+)\\][. _].*?(?<year>19\\d{2}|20\\d{2})");
  }
  ...
  public static List<String> getEnhancedTvFileRegexps() {
    return Arrays.asList(
        ...
        "(?:^|[._ ])第\\s*(?<episode>\\d{1,4})\\s*集", // 阿拉伯数字集数：第01集 / 第 12 集
  ...
  public static List<String> getRegexPriorityConfig() {
    return Arrays.asList(
        ...
        "^(?<title>[\\u4e00-\\u9fff]+.*?)[. _](?<year>19\\d{2}|20\\d{2})",
        "^\\[(?<title>[^\\[\\]]+)\\][. _].*?(?<year>19\\d{2}|20\\d{2})",
        "^(?<title>.+?)[. _](?<year>19\\d{2}|20\\d{2})[. _](?<title2>.*)$",
        "[._ ]第[一二三四五六七八九十]+集",
        "[._ ]第[一二三四五六七八九十]+季",
