...
import com.hienao.openlist2strm.job.ConfigDeletionCleanupJob;
...
public class ApplicationStartupListener implements ApplicationListener<ApplicationReadyEvent> {
  ...
  public void onApplicationEvent(ApplicationReadyEvent event) {
    ...
    try {
      ...
      // 3.5 注册账号删除到期清理定时任务
      registerConfigDeletionCleanupTask();
  ...
  /** 注册账号删除到期清理定时任务（每天凌晨 2:30 执行，物理清理软删除超过 7 天的账号数据）。 */
  private void registerConfigDeletionCleanupTask() {
    try {
      Scheduler scheduler = quartzSchedulerService.getScheduler();

      String cronExpression = "0 30 2 * * ?";
      String jobName = "ConfigDeletionCleanupJob";
      String jobGroup = "SYSTEM";

      JobDetail jobDetail =
          JobBuilder.newJob(ConfigDeletionCleanupJob.class)
              .withIdentity(jobName, jobGroup)
              .withDescription("账号删除到期清理定时任务")
              .storeDurably(true)
              .build();
      ...
      log.info("账号删除到期清理定时任务注册成功，执行时间: 每天凌晨2:30");

    } catch (Exception e) {
      log.error("注册账号删除到期清理定时任务失败: {}", e.getMessage(), e);
    }
  }
