...
public class TmdbIdExtractor {
  ...
  /** 宽松 TMDB ID 模式：tmdb-数字 / tmdb数字 / tmdbid-数字（无花括号），例如 tmdb-20342。 */
  private static final Pattern LOOSE_TMDB_ID_PATTERN =
      Pattern.compile("(?i)(?:\\btmdb(?:id)?[-_\\s]*)(\\d+)");

  /**
   * 从相对路径中提取TMDB ID
   *
   * <p>支持两种格式：{@code {tmdbid-数字}}（刮削目录命名）与 {@code tmdb-数字}（用户目录命名），
   * 后者常见于媒体库目录如「不能说的秘密 tmdb-20342」。
   *
  ...
  public static Integer extractTmdbIdFromPath(String relativePath) {
    ...
    try {
      ...
      matcher = LOOSE_TMDB_ID_PATTERN.matcher(relativePath);
      if (matcher.find()) {
        int tmdbId = Integer.parseInt(matcher.group(1));
        log.debug("从路径中提取到TMDB ID(宽松): {} -> {}", relativePath, tmdbId);
        return tmdbId;
      }
