...
public class TmdbApiService {
  ...
  private void logRequestDetails(String method, String url, Map<String, String> params) {
    log.info("TMDB API 请求 - 方法: {}, URL: {}", method, sanitizeUrl(url));
  ...
  private String sanitizeUrl(String url) {
    return url == null ? null : url.replaceAll("(?i)([?&]api_key=)[^&]*", "$1***");
  }
   ...
   * @return 演员表信息
   */
  public TmdbCredits getCredits(String mediaType, Integer mediaId) {
    Map<String, Object> tmdbConfig = systemConfigService.getTmdbConfig();
    String apiKey = (String) tmdbConfig.get("apiKey");
    if (apiKey != null) {
      apiKey = apiKey.trim();
    ...
    try {
      ...
      url =
          UriComponentsBuilder.fromHttpUrl(baseUrl + "/" + mediaType + "/" + mediaId + "/credits")
              .queryParam("api_key", apiKey)
              .queryParam("language", language)
              .toUriString();

      Map<String, String> requestParams = new java.util.HashMap<>();
      requestParams.put("mediaType", mediaType);
      requestParams.put("mediaId", String.valueOf(mediaId));
      logRequestDetails("GET", url, requestParams);
      ...
      TmdbCredits credits = objectMapper.readValue(responseBody, TmdbCredits.class);
      log.info("获取演员表成功: {} type={}, id={}", mediaType, mediaId,
          credits.getCast() == null ? 0 : credits.getCast().size());

      return credits;

    } catch (Exception e) {
      logErrorDetails("GET", url, e, responseBody);
      throw new BusinessException("获取演员表失败: " + e.getMessage());
    }
  }
