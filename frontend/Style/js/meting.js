...
class MetingJSElement extends HTMLElement {
  ...
  _parse() {
    ...
    fetch(a)
      .then(res => {
        if (!res.ok) {
          console.error("MetingJS network error:", res.status, res.statusText);
      ...
      .then(data => {
        if (!Array.isArray(data)) {
          console.error("MetingJS: API response is not an array", data);
          return;
        }
        // 兼容 APlayer 格式
        const parsed = data.map(item => {
          return _objectSpread({}, item, {
            ...
            lrc: item.lrc || item.lyric || item.lrcContent || '',
            theme: item.theme || ''
          });
        });
        this._loadPlayer(parsed);
      })
      .catch(err => {
        console.error("MetingJS Fetch Error:", err);
      });
  }
  _loadPlayer(a) {
    ...
    if (a.length) {
      ...
      // 将 volume 等数字属性从字符串转换为数字
      if (a.volume !== undefined) {
        a.volume = parseFloat(a.volume);
      } else {
        a.volume = 1.0; // 默认音量 100%
      }
