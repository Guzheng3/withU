...
; (function (window, $) {
    ...
    const Navigation = {
        ...
        _updateNavPosition() {
            ...
            // 诗句淡出比吸顶提前一点点开始（提前 60px），
            // 到达吸顶点时诗句已基本淡出，与吸顶节奏一致
            const poemFadeTrigger = triggerPoint - 60;
            const shouldFadePoem = scrollTop > poemFadeTrigger;
        ...
        async _refresh() {
            ...
            try {
                ...
                this._pendingRequest = fetch(_siteBase + 'services/weather.php?mode=ip' + _wtParam, {
                    ...
                    .catch(() => fetch(_siteBase + 'services/weather.json', {
                        method: 'GET',
                        cache: 'default'
                    }))
                    .then(response => {
