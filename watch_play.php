...
if (!$media) { header('Location: /watch.php'); exit; }
...
$initialResolveUrl = withu_media_resolve_url($media);
?>
<!doctype html>
...
<style id="withu-player-overrides">
 ...
 .art-video-player .art-bottom .art-controls{margin:0 .35rem .35rem;padding:0 .35rem;border:1px solid rgba(255,255,255,.24);border-radius:10px;background:rgba(255,255,255,.14);box-shadow:inset 0 1px 0 rgba(255,255,255,.2),0 8px 26px rgba(0,0,0,.22);backdrop-filter:blur(18px) saturate(155%);-webkit-backdrop-filter:blur(18px) saturate(155%)}
...
.withu-episode-list{gap:.25rem;min-height:0;overflow-y:auto;overscroll-behavior:contain;scrollbar-gutter:stable}
 .withu-episode-list .episode-btn{min-height:30px;padding:.25rem .4rem;font-size:.75rem;line-height:1.2;text-align:center}
...
.episode-list{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.45rem;flex:1 1 auto;min-height:0;max-height:none;overflow-y:auto;overflow-x:hidden;overscroll-behavior:contain;scrollbar-gutter:stable}
 ...
 .art-video-player.art-fullscreen .withu-episode-overlay,.art-video-player.art-fullscreen-web .withu-episode-overlay{left:50%;right:auto;top:auto;bottom:3.35rem;width:min(760px,calc(100% - 2rem));max-height:min(28vh,240px);transform:translate(-50%,12px) scale(.98);transform-origin:bottom center}
...
.withu-episode-list,.art-video-player.art-fullscreen .withu-episode-list,.art-video-player.art-fullscreen-web .withu-episode-list{display:flex!important;flex:1 1 auto!important;min-height:0!important;flex-direction:column!important;gap:.3rem!important;overflow-y:auto!important;overscroll-behavior:contain!important;scrollbar-gutter:stable!important}
 ...
 .withu-speed-menu-title{padding:.2rem .3rem .35rem;color:#cbd5e1;font-size:.7rem;text-align:center}
  ...
  .withu-episode-overlay{display:flex!important;flex-direction:column!important;width:min(210px,calc(100% - 1rem))!important;height:75%!important;max-height:75%!important;padding:.65rem .55rem .55rem!important;overflow:hidden!important;overscroll-behavior:contain!important;transform-origin:bottom center!important;will-change:opacity,transform;transition:opacity 180ms var(--withu-ease-out),transform 180ms var(--withu-ease-out),visibility 0s linear 180ms!important}
  .withu-episode-overlay .withu-episode-list{display:flex!important;flex-direction:column!important;gap:.28rem!important}
  .withu-episode-overlay .episode-btn{min-height:32px!important;text-align:center!important}
  .art-video-player.art-fullscreen .withu-episode-overlay,.art-video-player.art-fullscreen-web .withu-episode-overlay{width:min(210px,calc(100% - 1rem))!important;height:75%!important;max-height:75%!important}
  ...
  @media(prefers-reduced-transparency:reduce){.player-top,.player-stage,.episode-panel,.media-detail,.recommend-panel,.withu-chat-panel,.withu-side-chat-panel,.withu-danmaku-inline-form,.episode-launcher-pill,.withu-voice-control,.withu-side-chat-control,.art-video-player .art-bottom .art-controls{background:rgba(255,255,255,.96)!important;backdrop-filter:none!important;-webkit-backdrop-filter:none!important}.withu-chat-panel,.withu-side-chat-panel,.withu-danmaku-inline-form,.episode-launcher-pill,.withu-voice-control,.withu-side-chat-control{background:rgba(15,23,42,.94)!important}}
  ...
  .art-video-player .art-bottom .art-controls{--withu-input-width:220px;position:relative!important;min-width:0!important;background:linear-gradient(135deg,rgba(245,182,200,.22),rgba(255,255,255,.13))!important;border-color:rgba(245,182,200,.42)!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.24),0 10px 30px rgba(228,134,164,.18),0 8px 26px rgba(0,0,0,.2)!important}
  ...
  .withu-player-topbar{position:absolute;left:1.05rem;right:1.05rem;top:.9rem;z-index:10055;display:grid;grid-template-columns:minmax(0,1fr) auto minmax(0,1fr);align-items:center;gap:.55rem;min-width:0;height:42px;padding:.28rem .55rem;border:1px solid rgba(255,255,255,.42);border-radius:18px;background:linear-gradient(135deg,rgba(255,255,255,.24),rgba(255,255,255,.1));box-shadow:0 12px 34px rgba(0,0,0,.24),inset 0 1px 0 rgba(255,255,255,.34);backdrop-filter:blur(20px) saturate(170%);-webkit-backdrop-filter:blur(20px) saturate(170%);color:#fff;text-shadow:0 1px 3px rgba(0,0,0,.42);pointer-events:none;opacity:1;transform:translateY(0);transition:opacity 220ms var(--withu-ease-out),transform 220ms var(--withu-ease-out),visibility 0s linear 0s}
  ...
  .withu-player-topbar-time{justify-self:end;min-width:3.3rem;text-align:right;font-size:.82rem;font-weight:900;letter-spacing:.02em}
  .withu-player-topbar-time{grid-column:3;grid-row:1}
...
function positionPopupAboveControl(panel,control,gap){if(!panel||!control)return;var host=panel.parentElement||episodeBottomHost||episodeFullscreenHost||episodeOverlayHost;if(!host)return;var hostRect=host.getBoundingClientRect(),buttonRect=control.getBoundingClientRect(),safeGap=typeof gap==='number'?gap:8,availableAbove=Math.max(120,buttonRect.top-hostRect.top-safeGap-8);if(panel.classList.contains('withu-episode-overlay')){var episodeHeight=Math.min(Math.max(120,hostRect.height*.75),availableAbove);panel.style.setProperty('height',episodeHeight+'px','important');panel.style.setProperty('max-height',episodeHeight+'px','important');}else if(panel.classList.contains('withu-side-chat-panel')||panel.classList.contains('withu-chat-panel')){panel.style.setProperty('max-height',availableAbove+'px','important');}var panelWidth=panel.offsetWidth||Math.min(320,Math.max(160,hostRect.width-16));var left=buttonRect.left-hostRect.left+(buttonRect.width-panelWidth)/2,maxLeft=Math.max(8,hostRect.width-panelWidth-8);left=Math.max(8,Math.min(maxLeft,left));var bottom=hostRect.bottom-buttonRect.top+safeGap;panel.style.setProperty('left',left+'px','important');panel.style.setProperty('right','auto','important');panel.style.setProperty('top','auto','important');panel.style.setProperty('bottom',bottom+'px','important');panel.classList.add('is-anchored');}
