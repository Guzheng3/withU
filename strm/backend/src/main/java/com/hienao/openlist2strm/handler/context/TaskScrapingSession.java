...
public class TaskScrapingSession {
  ...
  private final Set<String> scrapedTvShowDirectories = ConcurrentHashMap.newKeySet();
  private final Map<String, Integer> tvShowTmdbIdByDirectory = new ConcurrentHashMap<>();
  ...
  /** 已生成整剧元数据（tvshow.nfo/poster/fanart）的剧集目录集合，用于同一剧集目录内去重。 */
  public Set<String> scrapedTvShowDirectories() {
    return scrapedTvShowDirectories;
  }

  /** 剧集目录 → 已识别的 TMDB ID，用于同一剧集目录内复用识别结果，避免逐集重复解析。 */
  public Map<String, Integer> tvShowTmdbIdByDirectory() {
    return tvShowTmdbIdByDirectory;
  }
}
