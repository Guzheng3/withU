...
public class OpenlistConfigDto {
  ...
  /** 认证方式：token-令牌认证（默认），password-账号密码认证 */
  @Size(max = 20, message = "认证方式长度不能超过20个字符") private String authType;

  /** 密码（仅 auth_type=password 时填写，用于自动刷新Token，保存后不回显） */
  @Size(max = 500, message = "密码长度不能超过500个字符") private String password;
